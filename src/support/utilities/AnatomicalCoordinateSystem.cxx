...
AnatomicalCoordinateSystem<ScalarType, Dimension>
...
::SetChangeOfBasisMatrix(const MatrixType &changeOfBasisMatrix) {
  if ((changeOfBasisMatrix.rows() != Dimension) || (changeOfBasisMatrix.columns() != Dimension)) {
    throw std::runtime_error(
        "In AnatomicalCoordinateSystem : The dimensions of the change of basis matrix are not correct");
  }
...
template<class ScalarType, unsigned int Dimension>
itk::SpatialOrientation::ValidCoordinateOrientationFlags AnatomicalCoordinateSystem<ScalarType, Dimension>
...
//	itk::SpatialOrientation::ValidCoordinateOrientationFlags label;
  ...
  for (int i = 0; i < codestr_sz; i++) {
    switch (codestr.at(i)) {
      case 'R': codestr.at(i) = 'L';
        break;
      case 'L': codestr.at(i) = 'R';
        break;
      case 'A': codestr.at(i) = 'P';
        break;
      case 'P': codestr.at(i) = 'A';
        break;
      case 'S': codestr.at(i) = 'I';
        break;
      case 'I': codestr.at(i) = 'S';
        break;
      default: // unknown orientation
  ...
  for (int j = 0; j < Dimension; j++) {
    ...
    switch (index) {
      ...
      case 2 : letter = (changeOfBasisMatrix(index, j) > 0.0) ? 'S' : 'I';
        break;
      default : throw std::runtime_error("In AnatomicalCoordinateSystem : Invalid change of basis matrix");
    }
    result[j] = letter;
  }
...
template
class AnatomicalCoordinateSystem<double, 2>;
template
class AnatomicalCoordinateSystem<double, 3>;

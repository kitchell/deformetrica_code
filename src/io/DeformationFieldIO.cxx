...
DeformationFieldIO<ScalarType, Dimension>
::DeformationFieldIO() {
  m_Factor = 5;
  m_Def = NULL;
...
template<class ScalarType, unsigned int Dimension>
void DeformationFieldIO<ScalarType, Dimension>::Update() {
  this->WriteDeformationField("output", true);
}

template<class ScalarType, unsigned int Dimension>
void DeformationFieldIO<ScalarType, Dimension>::WriteDeformationField(const std::string &fn, bool finalField) {
  if (m_Def != NULL) {
...
/*
    ...
    std::cerr
        << "DeformationFieldIO::control diffeomorphism (abstract deformation) object should have been set before calling WriteDeformationField!"
        << std::endl;
  }
...
template<class ScalarType, unsigned int Dimension>
bool DeformationFieldIO<ScalarType, Dimension>::WriteDeformationField3DImage(const std::string &fn, bool finalField) {
  MatrixType DD = m_Def->GetDataDomain();

  ScalarType img_spacing = m_Def->GetKernelWidth();
  ...
  std::shared_ptr<Landmark<ScalarType, Dimension>> ldmkObj = std::make_shared<Landmark<ScalarType, Dimension>>();
  ...
  std::shared_ptr<DeformableMultiObjectType> DMO = std::make_shared<DeformableMultiObjectType>();
  ...
  std::shared_ptr<DeformableMultiObjectType> UDMO, UDMO2;
  DeformableObjectList ulist, ulist2;

//	int NumbTimePoints = m_Def->GetNumberOfTimePoints();
  MatrixListType deformedFieldPoints(2); // for two consecutive timepoints
  ...
  typename itk::OrientImageFilter<DisplacementField3DType, DisplacementField3DType>::Pointer
      orienter = itk::OrientImageFilter<DisplacementField3DType, DisplacementField3DType>::New();
  ...
  for (int t = T0 + step; t < TN; t += step) {
    ...
    for (DeformableObjectListIterator it = ulist.begin(); it != ulist.end(); ++it) {
      std::shared_ptr<Landmark<ScalarType, Dimension>> uldmkObj;
      uldmkObj = std::static_pointer_cast<Landmark<ScalarType, Dimension>>(*it);
      deformedFieldPoints[0] = uldmkObj->GetPointCoordinates();
    }

    for (DeformableObjectListIterator it = ulist2.begin(); it != ulist2.end(); ++it) {
      std::shared_ptr<Landmark<ScalarType, Dimension>> uldmkObj2;
      uldmkObj2 = std::static_pointer_cast<Landmark<ScalarType, Dimension>>(*it);
    ...
    for (long i = 0; i < npoints; i++) {
      ...
      for (int m = 0; m < 3; m++) {
        orient_displacement[m] =
            (baseMtx(m, 0) * displacement[0]) + (baseMtx(m, 1) * displacement[1]) + (baseMtx(m, 2) * displacement[2]);
...
template class DeformationFieldIO<double,2>;
template class DeformationFieldIO<double,3>;

...
::EndElement(const char* name)
{
	...
	else if(itksys::SystemTools::Strucmp(name,"IMAGE-GRID-DOWNSAMPLING") == 0)
	{
		double d = atof(m_CurrentString.c_str());
		m_PObject->SetImageGridDownsampling(d);
	}
	...
	else if(itksys::SystemTools::Strucmp(name,"DATA-SIGMA-NORMALIZED-HYPERPARAMETER") == 0)
	{
	double d = atof(m_CurrentString.c_str());
	m_PObject->SetDataSigma_Normalized_Hyperparameter(d);
	}

	else if(itksys::SystemTools::Strucmp(name,"DATA-SIGMA-PRIOR") == 0)
	{
	double d = atof(m_CurrentString.c_str());
	m_PObject->SetDataSigma_Prior(d);
	}
	...
	else if(itksys::SystemTools::Strucmp(name,"ANATOMICAL-COORDINATE-SYSTEM") == 0)
	{
		m_PObject->SetAnatomicalCoordinateSystem(m_CurrentString);
	}
...
//	else if(itksys::SystemTools::Strucmp(name,"PARAMETRIC-TEMPLATE-IMAGE") == 0)
//	{
//		if (itksys::SystemTools::Strucmp(m_CurrentString.c_str(),"On") == 0)
//			m_PObject->SetUseParametricTemplateImage();
//		else if (itksys::SystemTools::Strucmp(m_CurrentString.c_str(),"Off") == 0)
//			m_PObject->UnsetUseParametricTemplateImage();
//		else
//			std::cout << "WARNING: Cannot read: " << m_CurrentString.c_str() << ", should be On or Off" << std::endl;
//	}
//	else if(itksys::SystemTools::Strucmp(name,"PHOTOMETRIC-KERNEL-WIDTH") == 0)
//	{
//		double d = atof(m_CurrentString.c_str());
//		m_PObject->SetPhotometricKernelWidth(d);
//	}
//	else if(itksys::SystemTools::Strucmp(name,"PHOTOMETRIC-KERNEL-TYPE") == 0)
//	{
//		m_PObject->SetPhotometricKernelType(m_CurrentString);
//	}
	else if(itksys::SystemTools::Strucmp(name,"PHOTOMETRIC-CP-SPACING") == 0)
	{
		double d = atof(m_CurrentString.c_str());
		m_PObject->SetPhotometricCPSpacing(d);
	}
}
...
::WriteFile()
{
	...
	WriteField<double>(this, "DATA-SIGMA-NORMALIZED-HYPERPARAMETER", p->GetDataSigma_Normalized_Hyperparameter(), output);
	WriteField<double>(this, "DATA-SIGMA-PRIOR", p->GetDataSigma_Prior(), output);
	...
	WriteField<double>(this, "IMAGE-GRID-DOWNSAMPLING", p->GetImageGridDownsampling(), output);

	WriteField<std::string>(this, "ANATOMICAL-COORDINATE-SYSTEM", p->GetAnatomicalCoordinateSystem(), output);
...
readDeformableObjectParametersXML(const char* fn)
{
	...
	//STANLEY
	DeformableObjectParameters::Pointer out = reader->GetOutputObject();
	out->SetXMLFileName(fn);
	return out;

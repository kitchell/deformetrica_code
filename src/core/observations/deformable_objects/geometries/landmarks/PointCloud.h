...
class PointCloud : public Landmark<ScalarType, Dimension>
{
 public:
  ...
  std::shared_ptr<PointCloud> DeformedObject(const MatrixType& LandmarkPoints) const {
	  return std::static_pointer_cast<PointCloud>(doDeformedObject(LandmarkPoints)); }

  /// Clones the object.
  std::shared_ptr<PointCloud> Clone() const { return std::static_pointer_cast<PointCloud>(doClone()); }
 ...
 protected:
  ...
  virtual std::shared_ptr<AbstractGeometryType> doDeformedObject(const MatrixType& LandmarkPoints) const {
	  return std::static_pointer_cast<AbstractGeometryType>(std::make_shared<PointCloud>(*this, LandmarkPoints)); }

  /// Clones the object.
  virtual std::shared_ptr<AbstractGeometryType> doClone() const {
	  return std::static_pointer_cast<AbstractGeometryType>(std::make_shared<PointCloud>(*this)); }

...
class ScalarRadialKernel
{
	...
	__device__ __forceinline__ void Eval2(TYPE* gammai, TYPE* xi, TYPE* yj, TYPE* betaj)
	{
		TYPE r2 = 0.0f;
		TYPE temp;
	...
	/// Computes \f$\gamma_i += f'(r)^ell \beta_j\f$.
	__device__ __forceinline__ void Graddim(TYPE* gammai, TYPE* xi, TYPE* yj, TYPE* betaj, unsigned int* dim)
	{
		TYPE r2 = 0.0f;
		TYPE xmy[DIMPOINT];
		for(int k=0; k<DIMPOINT; k++)
		...
		TYPE s = 2.0  * Rfun.Diff(r2);
		for(int k=0; k<DIMVECT; k++)
			gammai[k] += betaj[k] * s * xmy[*dim];
	}

	/// Computes \f$\gamma_i += f'(r)^ell \beta_j\f$.
	__device__ __forceinline__ void Grad_varlin(TYPE* gammai, TYPE* xi, TYPE* yj, TYPE* betaj)
	{
		TYPE r2 = 0.0f;
		TYPE xmy[DIMPOINT];
		for(int k=0; k<DIMPOINT; k++)
		...
		for(int k=0; k<DIMVECT; k++)
		{
			...
			{
				gammai[k*DIMPOINT+l] += betaj[k] * s * xmy[l];
			}
		}
	}

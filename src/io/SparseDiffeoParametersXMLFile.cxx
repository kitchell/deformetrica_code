...
::EndElement(const char* name)
{
	...
	else if(itksys::SystemTools::Strucmp(name,"T0") == 0)
	{
		float n = atof(m_CurrentString.c_str());
		m_PObject->SetT0(n);
	}
	else if(itksys::SystemTools::Strucmp(name,"TN") == 0)
	{
		float n = atof(m_CurrentString.c_str());
		m_PObject->SetTN(n);
	}
	...
	else if (itksys::SystemTools::Strucmp(name,"COVARIANCE-MOMENTA-NORMALIZED-HYPERPARAMETER") == 0)
	{
		double n = atof(m_CurrentString.c_str());
		m_PObject->SetCovarianceMomenta_Normalized_Hyperparameter(n);
	}
    else if(itksys::SystemTools::Strucmp(name,"ATLAS-TYPE") == 0)
    {
        m_PObject->SetModelType(m_CurrentString);
    }
    else if(itksys::SystemTools::Strucmp(name,"MAXIMUM-NUMBER-OF-CLASS") == 0)
    {
        int i = atoi(m_CurrentString.c_str());
        m_PObject->SetMaximumNumberOfClasses(i);
    }
	else if(itksys::SystemTools::Strucmp(name,"COVARIANCE-MOMENTA-PRIOR-INVERSE-FN") == 0)
	{
		m_PObject->SetCovarianceMomenta_Prior_Inverse_fn(m_CurrentString);
	}
	else if(itksys::SystemTools::Strucmp(name,"COVARIANCE-MOMENTA-INVERSE-FN") == 0)
	{
		m_PObject->SetCovarianceMomentaInverse_fn(m_CurrentString);
	}
	else if(itksys::SystemTools::Strucmp(name,"MODEL-NAME") == 0)
	{
		m_PObject->SetModelName(m_CurrentString);
	}
	...
	else if(itksys::SystemTools::Strucmp(name,"FREEZE-TEMPLATE") == 0)
	{
		if(itksys::SystemTools::Strucmp(m_CurrentString.c_str(),"ON") == 0)
			m_PObject->SetFreezeTemplate();
		else if (itksys::SystemTools::Strucmp(m_CurrentString.c_str(),"OFF") == 0)
			m_PObject->UnsetFreezeTemplate();
		else
			std::cout << "WARNING: Cannot read: " << m_CurrentString.c_str() << ", should be On or Off" << std::endl;
	}
	...
	else if(itksys::SystemTools::Strucmp(name,"INITIAL-STEP-SIZE") == 0)
	{
		double d = atof(m_CurrentString.c_str());
		m_PObject->SetInitialStepSize(d);
	}
	...
	else if(itksys::SystemTools::Strucmp(name,"COMPUTE-TRUE-INVERSE-FLOW") == 0)
	{
		if (itksys::SystemTools::Strucmp(m_CurrentString.c_str(),"On") == 0)
			m_PObject->SetComputeTrueInverseFlow();
		else if (itksys::SystemTools::Strucmp(m_CurrentString.c_str(),"Off") == 0)
			m_PObject->UnsetComputeTrueInverseFlow();
		else
			std::cout << "WARNING: Cannot read: " << m_CurrentString.c_str() << ", should be On or Off" << std::endl;
	}
}
...
::WriteFile()
{
	...
	WriteField<const char*>(this, "FREEZE-TEMPLATE", p->FreezeTemplate()?"On":"Off", output);
	...
	WriteField<double>(this, "COVARIANCE-MOMENTA-NORMALIZED-HYPERPARAMETER", p->GetCovarianceMomenta_Normalized_Hyperparameter(), output);

	WriteField<const char*>(this, "COMPUTE-TRUE-INVERSE-FLOW", p->ComputeTrueInverseFlow()?"On":"Off", output);
	...
	WriteField<double>(this, "INITIAL-STEP-SIZE", p->GetInitialStepSize(), output);

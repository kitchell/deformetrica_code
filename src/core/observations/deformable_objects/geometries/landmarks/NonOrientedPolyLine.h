...
class NonOrientedPolyLine : public Landmark<ScalarType, Dimension> {
 public:
  ...
  std::shared_ptr<NonOrientedPolyLine> DeformedObject(const MatrixType &LandmarkPoints) const {
    return std::static_pointer_cast<NonOrientedPolyLine>(doDeformedObject(LandmarkPoints));
  }

  /// Clones the object.
  std::shared_ptr<NonOrientedPolyLine> Clone() const {
    return std::static_pointer_cast<NonOrientedPolyLine>(doClone());
  }
  ...
  inline void SetKernelType(KernelEnumType kernelType) {
  ...
  inline void SetKernelWidth(ScalarType h) {
    m_KernelWidth = h;
    this->SetModified();
  }
  ...
  */
  ...
  virtual unsigned long GetDimensionOfDiscretizedObject() const {
    int d = 1;
    for (unsigned int dim = 0; dim < Dimension; dim++) {
      d *= floor((Superclass::Superclass::m_BoundingBox(dim, 1) - Superclass::Superclass::m_BoundingBox(dim, 0))
                     / m_KernelWidth + 1.0);
    }
    d *= Dimension;
    return d;
 ...
 protected:
  ...
  virtual std::shared_ptr<AbstractGeometryType> doDeformedObject(const MatrixType &LandmarkPoints) const {
    return std::static_pointer_cast<AbstractGeometryType>(std::make_shared<NonOrientedPolyLine>(*this, LandmarkPoints));
  }

  /// Clones the object.
  virtual std::shared_ptr<AbstractGeometryType> doClone() const {
    return std::static_pointer_cast<AbstractGeometryType>(std::make_shared<NonOrientedPolyLine>(*this));
  }

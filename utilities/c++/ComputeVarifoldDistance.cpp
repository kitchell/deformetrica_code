...
#include <src/core/observations/deformable_objects/geometries/landmarks/NonOrientedPolyLine.h>
#include <src/core/observations/deformable_objects/geometries/landmarks/NonOrientedSurfaceMesh.h>
...
int main(int argc, char **argv) {
  ...
  typedef NonOrientedSurfaceMesh<ScalarType, 3> NonOrientedSurfaceMeshType;

  /// Main.
  if (dimension == 2) {

    /// Reading the poly line data.
    std::shared_ptr<NonOrientedPolyLineType> pl1 = std::make_shared<NonOrientedPolyLineType>();
    ...
    std::shared_ptr<NonOrientedPolyLineType> pl2 = std::make_shared<NonOrientedPolyLineType>();
    ...
    std::shared_ptr<NonOrientedSurfaceMeshType> sm1 = std::make_shared<NonOrientedSurfaceMeshType>();
    vtkSmartPointer<vtkPolyDataReader> reader1 = vtkSmartPointer<vtkPolyDataReader>::New();
    reader1->SetFileName(pathToMesh1);
    reader1->Update();
    vtkSmartPointer<vtkPolyData> polyData1 = reader1->GetOutput();
    ...
    std::shared_ptr<NonOrientedSurfaceMeshType> sm2 = std::make_shared<NonOrientedSurfaceMeshType>();
    vtkSmartPointer<vtkPolyDataReader> reader2 = vtkSmartPointer<vtkPolyDataReader>::New();
    reader2->SetFileName(pathToMesh2);
    reader2->Update();
    vtkSmartPointer<vtkPolyData> polyData2 = reader2->GetOutput();
